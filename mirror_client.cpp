...
char handler_common_dir[200], handler_mirror_dir[200];
...
int main(int argc, char * argv[]){
  ...
  char common_dir[200], input_dir[200], mirror_dir[200], log_file[200];
  char any_path[100];
...
void write_to_logfile(int send, int send_bytes, int logfile){
  ...
  if(send==1)
    ...
    sprintf(tmp, "sended dir\n");
  else if(send==5)
    sprintf(tmp, "rcved dir\n");

  write(logfile, tmp, strlen(tmp));//check for error
  fdatasync(logfile);
...
int rvc_process(char* new_dot_id, int id, char* mirror_dir, int logfile, int buff_len){
  ...
  while(sz!=0){
    ...
    while(left_to_read>0){
      ...
      write_to_logfile(0, bytes_to_read, logfile);
    ...
    if(sz!=-1){//if it is a file
      //make file
      FILE *new_file=NULL;
      sprintf(tmp, "%s/%s/%s",mirror_dir,new_dot_id,tmp_file_name);
      ...
      sprintf(tmp, "%s/%s/%s",mirror_dir,new_dot_id,strchr(tmp_file_name+2,'/')+1);
